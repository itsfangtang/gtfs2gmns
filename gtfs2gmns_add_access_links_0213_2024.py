...
def find_nearest_stop_for_zones(zone_file_path, all_node_df):
    # 读取zone.csv文件
    zones_df = pd.read_csv(zone_file_path)
    # 初始化一个空列表来存储每个zone的最近stop信息
    nearest_stops_list = []

    # 解析geometry列以获取zone中心点坐标
    zones_df['zone_lat'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[1]))
    zones_df['zone_lon'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[0]))

    physical_node_df = all_node_df[all_node_df.node_id == all_node_df.physical_node_id]
    physical_node_df = physical_node_df.reset_index()

    for index, zone in zones_df.iterrows():
        # 计算每个zone到所有stop的距离
        distances = physical_node_df.apply(
            lambda x: _calculate_distance_from_geometry(zone['zone_lon'], zone['zone_lat'], x['x_coord'], x['y_coord']), axis=1)
        nearest_stop_index = distances.idxmin()
        access_stop_id = physical_node_df.iloc[nearest_stop_index]['physical_node_id']
        nearest_distance = distances.iloc[nearest_stop_index]

        # 将结果添加到列表中
        nearest_stops_list.append({
    ...
    # 使用列表创建DataFrame
    access_stops_df = pd.DataFrame(nearest_stops_list)
    return access_stops_df


def find_300m_stop_for_zones(zone_file_path, all_node_df):
    zones_df = pd.read_csv(zone_file_path)
    zones_df['zone_lat'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[1]))
    zones_df['zone_lon'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[0]))

    stops_within_zones_list = []
    physical_node_df = all_node_df[all_node_df.node_id == all_node_df.physical_node_id]
    physical_node_df = physical_node_df.reset_index()
    ...
    for index, zone in zones_df.iterrows():
        ...
        physical_node_df['distance'] = physical_node_df.apply(
            lambda x: _calculate_distance_from_geometry(zone_lon, zone_lat, x['x_coord'], x['y_coord']), axis=1)
        within_threshold_df = physical_node_df[physical_node_df['distance'] <= 300]

        for _, row in within_threshold_df.iterrows():
            stops_within_zones_list.append({
                'zone_id': zone_id,
                'access_stop_id': row['physical_node_id'],
                'distance': row['distance'],
                'zone_lon': zone_lon,
                'zone_lat': zone_lat
            })

    access_stops_df = pd.DataFrame(stops_within_zones_list)
    return access_stops_df

def find_500m_stop_for_zones(zone_file_path, all_node_df):
    zones_df = pd.read_csv(zone_file_path)
    zones_df['zone_lat'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[1]))
    zones_df['zone_lon'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[0]))
    ...
    for index, zone in zones_df.iterrows():
        ...
        physical_node_df['distance'] = physical_node_df.apply(
            lambda x: _calculate_distance_from_geometry(zone_lon, zone_lat, x['x_coord'], x['y_coord']), axis=1)
        within_threshold_df = physical_node_df[physical_node_df['distance'] <= 500]
...
def find_1000m_stop_for_zones(zone_file_path, all_node_df):
    zones_df = pd.read_csv(zone_file_path)
    zones_df['zone_lat'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[1]))
    zones_df['zone_lon'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[0]))
    ...
    for index, zone in zones_df.iterrows():
        ...
        physical_node_df['distance'] = physical_node_df.apply(
            lambda x: _calculate_distance_from_geometry(zone_lon, zone_lat, x['x_coord'], x['y_coord']), axis=1)
        within_threshold_df = physical_node_df[physical_node_df['distance'] <= 1000]
...
def find_1609m_stop_for_zones(zone_file_path, all_node_df):
    zones_df = pd.read_csv(zone_file_path)
    zones_df['zone_lat'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[1]))
    zones_df['zone_lon'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[0]))
    ...
    for index, zone in zones_df.iterrows():
        ...
        physical_node_df['distance'] = physical_node_df.apply(
            lambda x: _calculate_distance_from_geometry(zone_lon, zone_lat, x['x_coord'], x['y_coord']), axis=1)
        within_threshold_df = physical_node_df[physical_node_df['distance'] <= 1609]
...
def find_2000m_stop_for_zones(zone_file_path, all_node_df):
    zones_df = pd.read_csv(zone_file_path)
    zones_df['zone_lat'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[1]))
    zones_df['zone_lon'] = zones_df['geometry'].apply(lambda x: float(x.split('(')[1].split(')')[0].split()[0]))
    ...
    for index, zone in zones_df.iterrows():
        ...
        physical_node_df['distance'] = physical_node_df.apply(
            lambda x: _calculate_distance_from_geometry(zone_lon, zone_lat, x['x_coord'], x['y_coord']), axis=1)
        within_threshold_df = physical_node_df[physical_node_df['distance'] <= 2000]
...
def gtfs2gmns(input_path, output_path):
    ...
    gtfs_folder_list = []
    zone_file_path = os.path.join('Bay_Area_zone.csv')
    ...
    access_stops = find_1000m_stop_for_zones(zone_file_path, all_node_df)
    # access links
    # Load the nearest stops data
    access_stops_df = access_stops

    # Initialize a list to hold the links data
    zone_stop_links_list = []

    for index, row in access_stops_df.iterrows():
        zone_id = row['zone_id']
        access_node_id = row['access_stop_id']
        distance = row['distance']
        zone_lon = row['zone_lon']
        zone_lat = row['zone_lat']

        # Assuming you want to keep the geometry of the link as a line from zone to stop
        # Find the coordinates of the zone and the stop
        stop_geometry = all_node_df.loc[all_node_df['node_id'] == access_node_id, 'geometry'].values[0]
        geometry_parts = stop_geometry.split('(')[1].split(')')[0].split()
        stop_lon = float(geometry_parts[0])
        stop_lat = float(geometry_parts[1])
        geometry = 'LINESTRING (' + str(zone_lon) + ' ' + str(zone_lat) + ', ' + \
                   str(stop_lon) + ' ' + str(stop_lat) + ')'
        ...
        link_dict = {
            ...
            'distance': distance,
            'geometry': geometry  # Placeholder for actual geometry
        }

        zone_stop_links_list.append(link_dict)

    # Convert the list of dictionaries to a DataFrame
    zone_stop_links_df = pd.DataFrame(zone_stop_links_list)
    # Save the zone-stop links to a CSV file
    zone_stop_links_df.to_csv(os.path.join(output_path, 'access_links_1000m.csv'), index=False)
...
if __name__ == '__main__':
    ...
    input_path = './GTFS/SF_copy'
    output_path = './GMNS/SF_copy'
    time_period_id = 1
